...
STATIC mp_obj_t mod_quirc_init(mp_obj_t hor_res_obj, mp_obj_t ver_res_obj) {
    ...
    // uint8_t* _framebuffer = heap_caps_malloc(CONFIG_QUIRC_QR_MAX_HOR_RES * CONFIG_QUIRC_QR_MAX_VER_RES * sizeof(uint8_t), MALLOC_CAP_INTERNAL);
    ...
    quirc_new();
    quirc_resize(&_quirc, hor_res_obj, ver_res_obj);
...
STATIC mp_obj_t mod_quirc_scan() {
    ...
    // DEBUG("%d\n", _framebuffer[(uint8_t)(60*CONFIG_QUIRC_QR_MAX_HOR_RES + 60)]);
...
STATIC mp_obj_t mod_quirc_load_framebuffer(size_t n_args, const mp_obj_t *args) {
    ...
    // DEBUG("swapped internal image pointer\n");

    // DEBUG("%d\n", _framebuffer[(60*CONFIG_QUIRC_QR_MAX_HOR_RES + 60)]);
